...
/*pipe () 创建一个管道，一个单向的数据通道，可以用于进程间通信。数组pipefd用于返回两个引用管道末端的文件描述符。pipefd[0]指的是管道的读取端。  pipefd[1]指到管道的写端。数据写入写入端管道由内核缓冲，直到从读取中读取管道的末端。成功时，返回零。出错时，返回 -1，errno为设置以指示错误，并且pipefd保持不变。*/
    int p1 = pipe(pi1);
    int p2 = pipe(pi2);
    long temptime;
...
并在读取连接两个字符串并通过fd2管道将字符串传递给父进程后退出。 */
	//// Parent process
        else
        {
            ...
            write(pi1[1], buf1, 25);
            //wait(rc);子进程不会退出，父进程会阻塞 
/* wait(NULL)将阻塞父进程，直到其任何子进程完成。如果子进程在父进程到达之前终止，wait(NULL)则子进程将变为僵尸进程，直到其父进程等待它并从内存中释放。
如果父进程不等待其子进程，而父进程先完成，则子进程将成为孤儿并被分配init为其子进程。而init会等待并释放进程表中的进程条目。换句话说：父进程将被阻塞，直到子进程向操作系统返回退出状态，然后操作系统返回给父进程。如果子进程在父进程到达 wait(NULL) 之前完成，那么它将读取退出状态，释放进程表中的进程条目并继续执行直到它也完成。*/
